...
(function() {
	 ...
	 * @static
	 **/
	platypus.buildDate = /*=date*/"Mon, 14 Sep 2015 20:51:20 GMT"; // injected by build process
...
//                if (!this.fullyCached && ((Math.abs(this.camera.x - vpL) > buffer) || (Math.abs(this.camera.y - vpT) > buffer)) && (this.imageMap.length > 0)) {
                if (!this.fullyCached && (cache.empty || !tempC.set(cache).setAll(tempC.x * this.tileWidth, tempC.y * this.tileHeight, tempC.width * this.tileWidth, tempC.height * this.tileHeight).contains(laxCam)) && (this.imageMap.length > 0)) {
                    ...
                    tempC.setAll(Math.round(laxCam.x / this.tileWidth - ctw2) + ctw2, Math.round(laxCam.y / this.tileHeight - cth2) + cth2, this.cacheTilesWidth, this.cacheTilesHeight);
                    if (tempC.left < 0) {
                        tempC.moveX(tempC.halfWidth);
                    } else if (tempC.right > this.tilesWidth) {
                        tempC.moveX(this.tilesWidth - tempC.halfWidth)
                    }
                    if (tempC.top < 0) {
                        tempC.moveY(tempC.halfHeight);
                    } else if (tempC.bottom > this.tilesHeight) {
